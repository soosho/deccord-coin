...
const char * const BITCOIN_CONF_FILENAME = "deccord.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //deccord-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Deccord
    // macOS: ~/Library/Application Support/Deccord
    // Unix-like: ~/.deccord
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Deccord";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Deccord";
#else
    // Unix-like
    return pathRet / ".deccord";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty deccord.conf if it does not exist
