...
impl PrivateKey {
    ...
    pub fn generate_deccord(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "deccord_helpers")]
    pub fn generate_deccord_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_deccord(rng))
            .collect()
    }
