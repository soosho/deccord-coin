...
The Deccord Core daemon can be configured to act as a trusted "border
router", implementing the deccord wire protocol and relay, making
...
The ZeroMQ feature in Deccord Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building deccordd:
...
For instance:
    ...
    $ deccordd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/deccordd.tx.raw \
...
These options can also be provided in deccord.conf.
...
From the perspective of deccordd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into deccordd directly. Furthermore, no information is
...
using. Deccordd appends an up-counting sequence number to each
